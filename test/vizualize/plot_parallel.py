...
def load_and_merge_data(file_pattern):
    ...
    for filepath in glob.glob(file_pattern):
        key = os.path.basename(filepath).split("_")[0]  # p[数字]を取得
...
def plot_combined_data(data, title):
    ...
    data = dict(sorted(data.items(), key=lambda x: int(x[0][1:])))
...
reten_data = load_and_merge_data("../p*_reten.txt")
plot_combined_data(reten_data, "Reten")

# match ファイルを1つのグラフにプロット
match_data = load_and_merge_data("../p*_match.txt")
plot_combined_data(match_data, "Match")
